...
import { AuthModal } from './components/AuthModal'; // my import model
...
export default function App() {
  ...
  const handleAddItem = async (newItem) => {
    try {
      ...
      // 2.Construct the object exactly as your Flask app (products.py) expects 
      const produtoParaBackend = {
        ...
        usuario_id: currentUser.id //Getting the REAL user ID 
      };

      // 3.Sends the POST request to the backend.
      const response = await axios.post("https://uni-desapego-d2od.onrender.com/api/products/", produtoParaBackend);

      // 4.If successful, the screen updates instantly with the ID generated by the database
      const itemSalvo = {
        ...newItem,
        id: response.data.id, 
        seller: {
      ...
      setIsNewItemModalOpen(false); //Closes the modal if it is open
  ...
  return (
    <div className="min-h-screen bg-[#F8F7FA] text-[#2D233F] pb-24 md:pb-12 font-sans selection:bg-[#F79EB1] selection:text-[#473469]">
      ...
      {/* Function it pass for the header open the model*/}
      <Header
        ...
        onOpenNewItemModal={() => {
          //If you're not logged in, it won't let you post a new item! 
          if (!currentUser) {
            setIsAuthModalOpen(true);
        ...
        //Passing state of user for the header
      ...
      {/* Here is the connected authentication modal */}
